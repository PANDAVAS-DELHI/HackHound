...
const Upper: React.FC = () => {
  ...
  const [hasMore, setHasMore] = useState<boolean>(false); // Track if more than 3 exist

  useEffect(() => {
    const fetchPrescriptions = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`${BACKEND_URL}cms/v1/doctor/prescription/patient/${patientId}?limit=4)`); 
        const fetchedPrescriptions = response.data.response || [];

        if (fetchedPrescriptions.length > 3) {
          setHasMore(true);
          setPrescriptions(fetchedPrescriptions.slice(0, 3)); // Only show 3
        } else {
          setHasMore(false);
      ...
      <div className="relative w-[1390px] p-2">
        ...
        <div className="flex-col gap-4 w-full bg-gray-100 border-b-blue-300 rounded-[31px] shadow-md">
          <div className="pt-2 space-y-4">
            ...
            {/* View More button only if more than 3 prescriptions exist */}
            {hasMore && (
              <div className="flex justify-center items-center my-4">
                <Button
                  onClick={() => navigate(`allprescriptions/?temp=${patientId}`)}
                  title="View More"
                  size="md"
                  variant="secondary"
                />
